...
router.post('/', (req, res) => {
    try {
        const data = req.body;
        const question = QuestionRepository_1.questionRepository.create({
...
router.get('/', (req, res) => {
    try {
        ...
        const questions = QuestionRepository_1.questionRepository.findAll(limit, offset);
        res.json(questions);
    }
    catch (error) {
...
router.get('/:id', (req, res) => {
    try {
        const question = QuestionRepository_1.questionRepository.findById(req.params.id);
        if (!question) {
...
router.get('/domain/:domain', (req, res) => {
    try {
        ...
        const questions = QuestionRepository_1.questionRepository.findByDomain(req.params.domain, limit);
        res.json(questions);
    }
    catch (error) {
